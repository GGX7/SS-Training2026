// Tree — parts / groups / sections / subsections
function TreeRow({ node, openIds, toggle, active, activeSub, onPick }){
  ...
  return (
    <li>
      <div
        className={"t-row" + (isActive ? " is-active" : "") + (locked ? " locked" : "")}
        onClick={() => {
          if (locked) return;
...
function buildTree(){
  ...
  return window.CURRICULUM.map(p => ({
    id:p.id, label:p.label, locked:p.locked,
    children: (p.groups||[]).map(g => ({
      id:g.id, label:g.label, locked:g.locked,
  ...
  return (
    <aside className="rail">
      <div className="rail-head">
        <div className="rail-title">SS-Training</div>
        <div className="rail-sub">強化学習研究 · 全32節</div>
      </div>
